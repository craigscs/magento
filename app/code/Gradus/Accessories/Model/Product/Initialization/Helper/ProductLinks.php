...
class ProductLinks
{
    ...
    {
        if(isset($links[self::TYPE_NAME]) && !$product->getAccessoriesReadonly()) {

            $links = (isset($links[self::TYPE_NAME])) ? $links[self::TYPE_NAME] : $product->getAccessoriesLinkData();
            if (!is_array($links)) {
                $links = [];
            }
